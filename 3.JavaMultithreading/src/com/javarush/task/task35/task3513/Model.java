...
public class Model {
    ...
    public void addTile() {
        if (!getEmptyTiles().isEmpty()) {
            int index = (int) (Math.random() * getEmptyTiles().size()); // получаем случайный индекс списка.
            getEmptyTiles().get(index).value = (Math.random() < 0.9 ? 2 : 4); // присваиваем вес полученной клетке.
        }
    }
    ...
    boolean compressTiles(Tile[] tiles) {
        boolean flag = false;
        Tile[] copyTiles = tiles.clone();
        List<Tile> notEmptyElements = new ArrayList<>();

        for (int i = 0; i < tiles.length; i++) {
            if (!tiles[i].isEmpty())
                notEmptyElements.add(tiles[i]);
        }
        for (int i = 0; i < tiles.length; i++) {
            tiles[i] = new Tile();
        }
        for (int i = 0; i < notEmptyElements.size(); i++) {
            tiles[i] = notEmptyElements.get(i);
        }
        for (int i = 0; i < tiles.length; i++) {
    ...
    private boolean mergeTiles(Tile[] tiles) {
        ...
        for (int i = 0; i < tiles.length - 1; i++) {
            if (tiles[i].value == tiles[i + 1].value) {
                ...
                score += valueT;
                if (valueT > maxTileScore) maxTileScore = valueT;
    ...
    // чтобы состояние игрового поля изменилось проверяем есть ли свободные клетки.
    ...
    private MoveEfficiency getMoveEfficiency(Move move) {
        move.move();
        int numEfficiency = getEmptyTiles().size();
        int scoreEfficiency = score;
        if (!hasBoardChanged()) return new MoveEfficiency(-1, 0, move);
        rollback();
        return new MoveEfficiency(numEfficiency, scoreEfficiency, move);
    }
